...
const Profile = () => {
  ...
  const {data:loggeduser, isError:loggedUsererror, isSuccess:loggedUserSuccess} = useGetUserByIdQuery(userid);
  const {data:feeds, isLoading:feedloading, isError:feederror, isSuccess:feedSuccess, refetch} = useGetFeedsbyidQuery(userid)
  let content;
  if(feedloading){
    content = <p>Loading..</p>
  }
    ...
    <>
      ...
      <div className="profile">
        ...
        <div className='userInfodiv'>
          ...
          <div>
          <p style={{paddingTop:"30%"}}>{showeduser?showeduser.feeds.length:"-"}</p>
        ...
        <div className='userContactdiv'>
          ...
          :  (
            ...
            :<button style={{backgroundColor:"red"}}>팔로우 취소</button>
          }
          <button>메세지</button>
          <button>+</button>
          </>
